...
* [ ] 1 Within App.js, import the useReducer hook, our application's reducer and initialState object.
* [ ] 2 Use useReducer hook to get access to the application state and the dispatch function.
...
* [ ] 3 Replace "X" with a reference to `state.operation` within the operation element.
* [ ] 4 Replace "0" with a reference to `state.memory` within the memory element.
* [ ] 5 Replace "0" with a reference to `state.total` when passing a value to our TotalDisplay component.
* [ ] 6 Check to see that your total, operation and memory display in the UI match your initialState (100, * and 100 respectively)
* [ ] 7 **Test** that you are connected to state by changing the initialState value in your reducer to:
```
...
* [ ] 16 Note the `APPLY_NUMBER` action case (in ./reducer/index.js) and `applyNumber` action creator (in ./actions/index.js). This action adds, multiplies or subtracts a number passed into the action creator.
* [ ] 17 Import the `applyNumber` action creator into `App.js.`
* [ ] 18 Remove or comment out the `addOne` event handler from the 1 button.
* [ ] 19 Create an eventhandler that takes in a number as an argument and dispatches `applyNumber` with it.
* [ ] 20 Attach that eventhandler to the 1 button's `onClick` method, passing in a 1 as an argument. (Remember that we pass a function into that click handler, not the execution of a function)
* [ ] 21 **Test** that clicking the one button still adds one to the total display on the browser.
* [ ] 22 Connect all other number buttons to your new event handler, passing in their respective values.
* [ ] 21 **Test** that clicking on each button will add its respective value to the total display.
...
* [ ] 22 Note the `CHANGE_OPERATION` action case (in `./reducer/index.js`). This reducer case takes in a operator value (+, * or -) and assigns it to state.
* [ ] 23 Create an action creator (in `./actions/index.js`) that takes in an operator as an argument and creates an action object with the type `CHANGE_OPERATION.`
* [ ] 24 Import in your new action creator into `App.js.`
* [ ] 25 Create and attach event handlers to the `+`, `-` and `*` buttons that dispatch your new action creator. Make sure you pass in the appropriate operator string in each case.
* [ ] 26 **Test** that you can successfully change operators and apply numbers in each case.
...
* [ ] 27 Within `./reducers/index,` add in a case for `CLEAR_DISPLAY`. Clear display should set the total value in state to 0.
* [ ] 28 Within `./actions/index,` add in an action creator and action string constant to for `CLEAR_DISPLAY`. Make sure to import that constant into your reducer file.
* [ ] 29 Within `App.js,` import in your clearDisplay action creator.
* [ ] 30 Create and connect an event handler to the "CE" button that dispatches your clearDisplay action creator.
* [ ] 31 **Test** that your clearDisplay button works as expected.
...
* [ ] 32 When `M+` is pressed, the current memory value should be set to the current total value. Test by seeing the result of memory in the UI.
* [ ] 33 When `MR` is pressed, the current memory value should be applied to the current total value(See the APPLY_NUMBER case). 34 Test by adding a value to memory and then seeing if the total updates correctly when pressed.
* [ ] 35 When `MC` is pressed, the current memory value should be set to zero. Test by adding a value to memory and then seeing the memory value reset to zero when pressed.

...
This simplified calculator adds the entire number selected, rather than adding digits into end of number. In the end of the process, your calculator should function as follows:
...
* [x] **Test** that clicking the one button still adds one to the total display on the browser. -> It doesn't because the base operation is multiplication 
...
* [x] When `MR` is pressed, the state's current total value should be set to the current memory value (See the APPLY_NUMBER case). Test by adding a value to memory and then seeing if the total updates correctly when pressed.
